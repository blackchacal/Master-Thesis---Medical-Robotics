...
\documentclass[
  ...
  biblatex={            % Options for biblatex (see biblatex documentation)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     %%% uncomment for 'ieee' like citations
    backend=bibtex,   % use bibtex if possible
    style=ieee, % numeric(*), alphabetic, authoryear, bwl-FU
    sortcites=true
    sorting=none,      % none, nyt(*), ynt
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%% other options for biblatex
    maxbibnames=5,     % Never use 'et al' in the bibliography
...
\usepackage{wrapfig}     % To position images and text side by side. 
\usepackage{adjustbox}   % To widen the tables and center
\usepackage{pdfpages}    % Include pdf files
\usepackage{multirow}    % Tables multirow
\usepackage[flushleft]{threeparttable} % tables with bottom notes
...
\title[Semi-Autonomous 3D Positioning System \\Using Collaborative Robotics for 3D Bioprinting \\Directly on Burn Wounds]{}
%\subtitle{Science is beautiful}

% Autor
% use [f] for female and [m] for male
\authorname[m]{Ricardo Jorge Ribeiro Tonet}{Ricardo Tonet}
% \authordegree{Licenciatura em Ciências de Engenharia Biomédica}
\authordegree{Bachelor of Science in Biomedical Engineering}
...
% use [f] for female and [m] for male
\adviser[m]{Prof. Dr. Rui Cortesão}{Professor Auxiliar}{\\& Instituto de Sistemas e Robótica, Universidade de Coimbra} 
...
% use [f] for female and [m] for male
\coadviser[m]{Prof. Dr. José Manuel Fonseca}{Professor Associado com Agregação}{Faculdade de Ciências e Tecnologia, \newline Universidade NOVA de Lisboa}
...
%\chapterfile{chapter7} bioprinting system
...
%\appendixfile{appendix6} % print head
\appendixfile{appendix7}
%\annexfile{annex1} % ros, not needed
\annexfile{annex2}
\annexfile{annex3}
%\annexfile{annex4} % print head


% BibTeX bibliography files. Can be used multiple times with a single file nae each time.
\addbibfile{references_final.bib}
% Remove "In" from article references.
\renewbibmacro{in:}{%
  \ifentrytype{article}{}{\printtext{\bibstring{in}\intitlepunct}}}
% Remove issn field from article references.
